...
class LoanRepository(ILoanRepository):
    ...
    def apply_loan(self, loan):
        try:
            ...
            # Insert query with additional fields for HomeLoan or CarLoan
            query = """
                INSERT INTO Loan 
            ...
            if loan_id:
                ...
                raise Exception("Loan ID could not be retrieved.")
    ...
    def loan_status(self, loan_id):
        try:
            # Fetch customer_id from Loan table using loan_id
            self.cursor.execute("SELECT customer_id FROM Loan WHERE loan_id = ?", (loan_id,))
            loan = self.cursor.fetchone()
            
            if loan:
                customer_id = loan[0]
                print(f"Customer ID: {customer_id}")  # Debugging print

                # Fetch credit_score from Customer table using customer_id
                self.cursor.execute("SELECT credit_score FROM Customer WHERE customer_id = ?", (customer_id,))
                result = self.cursor.fetchone()
                
                if result:
                    credit_score = result[0]
                    print(f"Credit Score: {credit_score}")  # Debugging print
        ...
        try:
            ...
            if loan:
                ...
                if loan_status == "Approved":
                    ...
                    print(f"Your EMI should be greater than {emi_amount}")
                    if amount < emi_amount:
                        print(f"Payment rejected: Amount is less than the EMI({emi_amount}).")
                    else:
